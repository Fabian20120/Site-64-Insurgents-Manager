...
async def system_stats(ctx):
    ...
    description = (
        f"🖥️ **System**\n"
        f"{space*4}• 🧩 Platform: {uname.system}\n"
        f"{space*4}• 📦 Release: {uname.release}\n"
        f"{space*4}• 📜 Version: {uname.version}\n"
        f"{space*4}• 🏗️ Architecture: {uname.machine}\n"
        f"{space*4}• ⏰ Boot Time: {boot_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"{space*4}• ⏳ Uptime: {str(uptime).split('.')[0]}\n"
        f"{space*4}• 💻 Hostname: {host}\n"
        f"{space*4}• 🐍 Python Version: {py_ver}\n"
        f"{space*4}• ⚙️ Python Compiler: {py_comp}\n\n"

        f"🧠 **CPU**\n"
        f"{space*4}• ⚙️ Physical Cores: {cpu_count}\n"
        f"{space*4}• 🔢 Logical Cores: {cpu_count_logical}\n"
        f"{space*4}• 📊 CPU Usage (%): {cpu_percent}\n"
        f"{space*4}• 📈 Load Average: {load_avg}\n"
        f"{space*4}• 🧮 Cores Usage: \n{'; '.join([f'{space*8}→ 🟢 Core {i}: {usage}%\n' for i, usage in enumerate(cpu_usages)])}\n\n"

        f"🗄️ **Memory**\n"
        f"{space*4}• 🗄️ Total: {mem.total/1024**3:.2f} GB\n"
        f"{space*4}• 🟢 Available: {mem.available/1024**3:.2f} GB\n"
        f"{space*4}• 🔴 Used: {mem.used/1024**3:.2f} GB\n"
        f"{space*4}• 🟡 Free: {mem.free/1024**3:.2f} GB\n"
        f"{space*4}• 📊 Percent Used: {mem.percent}%\n"
        f"{space*4}• 💾 Swap Total: {swap.total/1024**2:.2f} MB\n"
        f"{space*4}• 💾 Swap Used: {swap.used/1024**2:.2f} MB\n"
        f"{space*4}• 💾 Swap Free: {swap.free/1024**2:.2f} MB\n"
        f"{space*4}• 💾 Swap Percent: {swap.percent}%\n\n"

        f"💽 **Disk**\n"
        f"{space*4}• 💽 Total: {disk.total/1024**3:.2f} GB\n"
        f"{space*4}• 💽 Used: {disk.used/1024**3:.2f} GB\n"
        f"{space*4}• 💽 Free: {disk.free/1024**3:.2f} GB\n"
        f"{space*4}• 📊 Percent: {disk.percent}%\n"
        f"{space*4}• 📖 Reads: {disk_io.read_count}\n"
        f"{space*4}• ✍️ Writes: {disk_io.write_count}\n"
        f"{space*4}• 📖 Read Bytes: {disk_io.read_bytes/1024**2:.2f} MB\n"
        f"{space*4}• ✍️ Write Bytes: {disk_io.write_bytes/1024**2:.2f} MB\n\n"

        f"🌐 **Network**\n"
        f"{space*4}• 📤 Bytes Sent: {net_io.bytes_sent/1024**2:.2f} MB\n"
        f"{space*4}• 📥 Bytes Received: {net_io.bytes_recv/1024**2:.2f} MB\n"
        f"{space*4}• 📦 Packets Sent: {net_io.packets_sent}\n"
        f"{space*4}• 📦 Packets Received: {net_io.packets_recv}\n"
    )
